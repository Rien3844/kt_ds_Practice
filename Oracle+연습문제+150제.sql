...
-- 1. 모르는 것 : Seattle의 지역번호가 뭔지 모른다. => 1700
SELECT LOCATION_ID
  FROM LOCATIONS
 WHERE CITY = 'Seattle'
;
-- 2. 모르는 것 : 'Seattle'에 있는 부서들의 번호가 무엇인지 모른다.
...
-- 1. 현재 시간을 조회한다.
-- ORACLE 전용 (ANSI X)
-- SYSDATE : 컬럼이 아님. 현재 시간을 가리키는 ORACLE 전용 예약어
SELECT SYSDATE 
  FROM DUAL
;

-- 현재 날짜를 기준으로 2달 이후의 날짜와 시간을 조회.
-- ADD_YEARS는 없음, 1년을 더하고싶으면 12를 적으면됨.
...
SELECT EMPLOYEE_ID
 ...
 WHERE HIRE_DATE BETWEEN TO_DATE('2005-09-01', 'YYYY-MM-DD')
   AND TO_DATE('2005-10-01', 'YYYY-MM-DD') - 1
;
...
-- 27. 이름이 4글자인 사원의 EMPLOYEE_ID를 조회한다.
SELECT EMPLOYEE_ID
  FROM EMPLOYEES
...
--내 상사의 월급?
SELECT SALARY
  FROM EMPLOYEES -- 내가 101번이면 상사 == 100
...
-- 65. 연봉이 2500, 3500, 7000 이 아니며 직업이 SA_REP 이나 ST_CLERK 인 사원들의 월급과 직무아이디를 조회한다.
-- 연봉이 조건이 맞으면서 직업도 조건에 맞아야하기에 직업을 괄호로 묶어줌. 
SELECT SALARY
     , JOB_ID
  FROM EMPLOYEES
 WHERE SALARY != 2500
...
-- 월급이 2500, 3500, 7000이 아닌 사원들의 사원번호와 월급을 조회한다.
--같은 CALUMN에서 여러 값을 조회할 경우 IN(조회할 값)으로 표현 가능.
SELECT EMPLOYEE_ID
     , SALARY
  FROM EMPLOYEES
 WHERE SALARY NOT IN (2500, 3500, 7000)
...
--직무 아이디가 SA_REP이거나 ST_CLERK인 사원들의 사원번호와 직무아이디를 조회한다.
SELECT EMPLOYEE_ID
     , JOB_ID
  FROM EMPLOYEES
...
-- 70. 모든 사원들을 정보를 연봉 오름차순 정렬하여 조회한다.
SELECT EMPLOYEE_ID
     , FIRST_NAME 
     , LAST_NAME 
     , EMAIL
...
-- 88. 직무번호 별 평균연봉을 평균연봉순으로 오름차순 정렬하여 조회한다.
SELECT JOB_ID
...
-- 89. 부서번호 별 평균연봉을 내림차순 정렬하여 조회한다.
SELECT DEPARTMENT_ID
     , AVG(SALARY) AS AVG_SALARY
  FROM EMPLOYEES
 GROUP BY DEPARTMENT_ID
...
SELECT TO_DATE('20230222', 'YYYY-MM-DD HH24:MI:SS')
  FROM DUAL
;
...
SELECT COUNTRY_ID
 ...
 --국가 명이 'a' 로 끝나지 않는 국가의 모든 정보를 조회한다
SELECT COUNTRY_ID
     , COUNTRY_NAME
     , REGION_ID
  FROM COUNTRIES
